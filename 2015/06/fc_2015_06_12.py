...
#
# Free Coding session for 2015-06-12
...
INFLATION_RATE = 0.03
INTEREST_RATE = 0.03  # loans can be short term (1 year) so hopefully lower cost

KWH_PRICE = 0.09  # 2015 dollars
...
# 90& of revenue from panels is used to pay the bond yield. 10% for profit/service costs
OUR_BOND_MARGIN = 0.95
df['Bond Sales'] = df['EOY net new capacity (KW)'] * annual_kwh_per_kw * df['Price Per KWH'] * (1/BOND_YIELD) * OUR_BOND_MARGIN
df['Bond Repayment'] = (df['Bond Sales'].shift(BOND_MATURITY) + (df['Bond Sales'].shift(BOND_MATURITY) * BOND_YIELD * BOND_MATURITY)).fillna(0)
df['Revenue'] = df['KWH generated'] * df['Price Per KWH'] + df['Bond Sales']
...
df['capital required per year'] = df['Capital for installations'] + df['warehouse costs'] + df['vehicles'] + df['salaries'] + df['gas'] + df['tools'] + df['computers'] + df['hosting'] + df['electricity'] + df['Bond Repayment']
